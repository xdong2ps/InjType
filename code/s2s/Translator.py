...
class Translator(object):
    ...
    def buildData(self, srcBatch, bioBatch, featsBatch, goldBatch, guideBatch):
        ...
        return s2s.Dataset(srcData, bioData, featsData, tgtData, None, None, self.opt.batch_size, self.opt.cuda, guideData)
    ...
    def translateBatch(self, srcBatch, bioBatch, featsBatch, tgtBatch, guideData):
        batchSize = srcBatch[0].size(1)
        beamSize = self.opt.beam_size
        #  (1) run the encoder on the src
        encStates, context, backward_hids, wordEmb = self.model.encoder(srcBatch, bioBatch, featsBatch, guideData)
