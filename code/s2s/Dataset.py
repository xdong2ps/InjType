...
class Dataset(object):
    ...
    def __getitem__(self, index):
        ...
        if tgtBatch is None:
            batch = zip(indices, srcBatch, bioBatch, *featBatches, guideBatch)
        else:
            if self.copySwitch is not None:
                batch = zip(indices, srcBatch, bioBatch, *featBatches, tgtBatch, copySwitchBatch, copyTgtBatch, guideBatch)
            else:
                batch = zip(indices, srcBatch, bioBatch, *featBatches, tgtBatch, guideBatch)
        # batch = zip(indices, srcBatch) if tgtBatch is None else zip(indices, srcBatch, tgtBatch)
        batch, lengths = zip(*sorted(zip(batch, lengths), key=lambda x: -x[1]))
        if tgtBatch is None:
            indices, srcBatch, bioBatch, *featBatches, guideBatch = zip(*batch)
        else:
            if self.copySwitch is not None:
                indices, srcBatch, bioBatch, *featBatches, tgtBatch, copySwitchBatch, copyTgtBatch, guideBatch = zip(*batch)
            else:
                indices, srcBatch, bioBatch, *featBatches, tgtBatch, guideBatch = zip(*batch)
    ...
    def shuffle(self):
        data = list(
            zip(self.src, self.bio, self.feats, self.tgt, self.copySwitch, self.copyTgt, self.guide))
        self.src, self.bio, self.feats, self.tgt, self.copySwitch, self.copyTgt, self.guide = zip(
