...
def generate_copy_loss_function(g_outputs, c_outputs, g_targets,
    ...
    total_loss = g_loss + nlu_loss * 1.5 + cl_loss * 2.5
    report_loss = total_loss.item()
    return total_loss, report_loss, 0, g_loss, cl_loss, nlu_loss
...
def load_dev_data(translator, src_file, bio_file, feat_files, tgt_file, guide_file):
    dataset, raw = [], []
    ...
    for line, tgt, guide in addPair(srcF, tgtF, guideF):
        if (line is not None) and (tgt is not None) and (guide is not None):
            ...
            if len(src_batch) == 0:
                break
        data = translator.buildData(src_batch, bio_batch, feats_batch, tgt_batch, guide_batch)
        dataset.append(data)
...
def evalModel(model, translator, evalData):
    ...
    for batch, raw_batch in zip(processed_data, raw_data):
        #  (1) read src and tgt (bio and feats are part of the legacy code and are never used in this project)
        src, bio, feats, tgt, guide, indices = batch[0]
        src_batch, tgt_batch = raw_batch

        #  (2) translate
        pred, predScore, predIsCopy, predCopyPosition, attn, _ = translator.translateBatch(src, bio, feats, tgt, guide)
...
def trainModel(model, translator, trainData, validData, testData, dataset, optim):
    ...
    def trainEpoch(epoch):
        ...
        for i in range(len(trainData)):
            ...
            totalBatchCount += 1
            """
            [0] ([0][0] wrap(srcBatch), [0][1] lengths), [1] ([1][0] wrap(bioBatch), [1][1] lengths), [2] ([2][0] (wrap(x) for x in featBatches), [2][1] lengths),
            [3] ([3][0] wrap(tgtBatch), [3][1] wrap(copySwitchBatch), [3][2] wrap(copyTgtBatch)),
            [4] [4] indices (not used) # original indices will be excluded !
            """
            ...
            '''
            g_outputs, c_outputs, c_gate_values, g_hiddens, nlu_outputs, type_emb = model(batch, False)
            targets = batch[3][0][1:]  # exclude <s> from targets
            copy_switch = batch[3][1][1:]
            c_targets = batch[3][2][1:]
            g_guides = batch[4][0]

            loss, res_loss, num_correct, g_loss, cl_loss, nlu_loss = generate_copy_loss_function(
            ...
            if testData is not None and totalBatchCount % opt.eval_per_batch == 0 and totalBatchCount >= opt.start_eval_batch:
                ...
                logger.info('Test Score: %g' % (test_bleu * 100))
        print('epoch g loss: ')
        print(epoch_loss)
        print('epoch cl loss: ')
        print(epoch_cl_loss)
        print('epoch nlu loss: ')
        print(epoch_nlu_loss)
...
def main():
    ...
    dataset = prepare_data_online(opt.train_src, opt.src_vocab, opt.train_bio, opt.bio_vocab, opt.train_feats,
                                  opt.feat_vocab, opt.train_tgt, opt.tgt_vocab, opt.train_guide_src, opt.guide_src_vocab)

    trainData = s2s.Dataset(dataset['train']['src'], dataset['train']['bio'], dataset['train']['feats'],
                            dataset['train']['tgt'], dataset['train']['switch'], dataset['train']['c_tgt'], opt.batch_size, opt.gpus, dataset['train']['guide_src'])
    ...
    if opt.dev_input_src and opt.dev_ref:
        validData = load_dev_data(translator, opt.dev_input_src, opt.dev_bio, opt.dev_feats, opt.dev_ref, opt.dev_guide_src)

    testData = None
    if opt.test_input_src and opt.test_ref:
        testData = load_dev_data(translator, opt.test_input_src, opt.test_bio, opt.test_feats, opt.test_ref, opt.test_guide_src)
